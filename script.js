(function() {
  ...
  // a. JavaScript Obfuscation (balik string lalu Base64)
  function encodeJSObfuscate(str) {
      return btoa(str.split("").reverse().join(""));
  }

  // b. Punycode (dummy: awali dengan "xn--" + Hex Encoding)
  function encodePunycode(str) {
      return "xn--" + encodeHex(str);
  }

  // c. Brainfuck Encoding
  // (Metode naive: untuk tiap karakter, tambahkan '+' sebanyak nilai ASCII lalu '.')
  ...
  // e. DNA Encoding
  // Setiap 2-bit diubah menjadi nukleotida: 00→A, 01→C, 10→G, 11→T.
  ...
  function processEncodings(input) {
      return {
          "Base64": tryEncode(() => encodeBase64(input)),
          "CharCodes": tryEncode(() => encodeCharCodes(input)),
          "URL": tryEncode(() => encodeURL(input)),
          "Morse": tryEncode(() => encodeMorse(input)),
          "Binary": tryEncode(() => encodeBinary(input)),
          "Rot13": tryEncode(() => encodeRot13(input)),
          "Hex": tryEncode(() => encodeHex(input)),
          "Atbash": tryEncode(() => encodeAtbash(input)),
          "Base85": tryEncode(() => encodeBase85(input)),
          "Base91": tryEncode(() => encodeBase91(input)),
          "Obfuscated": {
              "JSObfuscate": tryEncode(() => encodeJSObfuscate(input)),
              "Punycode": tryEncode(() => encodePunycode(input)),
              "Brainfuck": tryEncode(() => encodeBrainfuck(input)),
              "Base4096": tryEncode(() => encodeBase4096(input)),
              "DNA": tryEncode(() => encodeDNA(input))
          }
      };
  }

  // Fungsi pembantu untuk menangkap error pada setiap encoding
  function tryEncode(fn) {
    try {
      return fn();
    } catch (error) {
  ...
  // === Event Listener untuk tombol Encode dengan penanganan error ===
